...
async function createTemplate(req, res) {
    if (!req.body.serviceName) return res.json({
        message: 'missing service name',
        success: false,
        data: null
    });
    if (!req.body.price) return res.json({
        message: 'please include a price',
        success: false,
        data: null
    });
...
async function updateTemplate(req, res) {
    try {
        ...
        return res.json({
            message: `template has been ${result.active ? 'activated' : 'paused'}`,
            success: true,
            data: result
        });
    } catch (err) {
